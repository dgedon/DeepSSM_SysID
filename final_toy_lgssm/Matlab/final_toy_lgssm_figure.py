...
if __name__ == "__main__":
    ...
    if options['do_test']:
        ...
        muSTORN = y_sample_mu.squeeze()[:n]
        p3sigmaSTORN = muSTORN + 3 * y_sample_sigma.squeeze()[:n]
        m3sigmaSTORN = muSTORN - 3 * y_sample_sigma.squeeze()[:n]
        ...
        data = {
            ...
            'muSTORN': muSTORN,
            'p3sigmaSTORN': p3sigmaSTORN,
            'm3sigmaSTORN': m3sigmaSTORN,
        }
        df = pd.DataFrame(data) # , columns=['muTest', 'sigmaTest'])

        path = os.getcwd() + '/final_toy_lgssm/' + 'toy_lgssm_data.csv'
